# app.py
# Streamlit prototype: AI-Driven LCA Tool (India-centric) -- corrected and cleaned version
# Run with: streamlit run app.py
# Note: numeric values are illustrative defaults. Replace with validated data for production.
...
st.write(
    "These baseline factors are placeholders for demo. Replace them with validated LCI data for final results."
)
...
default_factors = {
    ...
    # red mud generation per tonne aluminium (t red mud / t Al)
    "red_mud_t_per_t_aluminium": 1.5,
    # SO2 estimate per tonne copper smelted (kg SO2 / t Cu)
    "so2_kg_per_t_copper": 25.0,
    # transport emission per ton-km (kg CO2e)
    "transport_kgco2_per_tkm": 0.05,
    # recycling costs (USD per tonne)
...
with st.form(key="input_form"):
    ...
    with col2:
        energy_source = st.selectbox(
            "Energy source (select nearest)", ["Coal-based grid", "Mixed grid", "Renewable-heavy"]
        )
        transport_km = st.number_input("Transport distance (km)", min_value=0, max_value=5000, value=200)
        transport_tonnes = st.number_input(
            "Quantity to assess (tonnes of metal)", min_value=1, max_value=100000, value=1
        )

    with col3:
        eol_option = st.selectbox("End-of-life option", ["Landfill", "Recycling", "Reuse"])
        storage_practice = st.selectbox(
            "Storage / residue handling",
...
# ---------------------------
if run_button:
    # Baseline selection (kg CO2 per kg)
    if metal == "Aluminium":
        if production_route == "Virgin/Raw":
            ...
            baseline = (
                default_factors["aluminium_virgin_kgco2_per_kg"] * (100 - recycled_pct) / 100
        ...
        if production_route == "Virgin/Raw":
            ...
            baseline = (
                default_factors["copper_virgin_kgco2_per_kg"] * (100 - recycled_pct) / 100
                + default_factors["copper_recycled_kgco2_per_kg"] * recycled_pct / 100
            )

    # Ore quality adjustments (illustrative multipliers)
    red_mud_t = 0.0
    so2_kg_total = 0.0

    if metal == "Aluminium":
        # ore_quality text contains "High", "Medium", "Low" substrings
    ...
    # total per tonne (production + transport) [note: kgco2_per_kg * 1000 converts kg/kg -> kg/tonne]
    ...
    if storage_practice != "Proper authorized storage":
        flags.append(
            (
                "Storage practice",
                "⚠️ Not authorized/temporary storage — requires review under Hazardous & Other Wastes Rules (2016)",
            )
        )
